...
- **Plan UI** (`event/AvailabilityCalendar.tsx`, `event/EventModal.tsx`, `.plan-*` in `styles.css`; iOS `Features/Events/Views/PlanStyle.swift`): styled after the landing's `EventFlowDemo` — rounded heat cells in 4 levels, your slots outlined, a glowing best slot, pill RSVP with counts. **Best slot rule** (`chat/lib/bestSlot.ts`, iOS `BestSlot`, must match): counts are live (others' saved rows + my local selection), highest count across all weeks, ties → earliest, none below 2 people. The creator's primary action is "Lock best time"; the date/time picker is the fallback.
