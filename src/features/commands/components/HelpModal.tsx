...
export default function HelpModal({ onClose }: Props) {
  return (
    ...
    >
      ...
      {/* Modal */}
      <div className="relative z-10 w-full max-w-lg bg-card rounded-2xl shadow-2xl shadow-black/30 border border-border max-h-[80vh] flex flex-col">
                ...
                {commands.map((cmd) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-text font-medium leading-snug">{cmd.description}</p>
                      {cmd.example && (
                        <p className="text-[11px] text-text-subtle font-mono mt-0.5 truncate">{cmd.example}</p>
                      )}
                    </div>
                  </div>
                ))}
