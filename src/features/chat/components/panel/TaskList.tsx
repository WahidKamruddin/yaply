    ...
    <>
      <div className="flex items-start gap-2.5 py-2.5 border-b border-border last:border-0">
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm ${isDone ? 'line-through text-text-subtle' : 'text-text'}`}>{task.title}</p>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <button
            ...
            className={`transition-colors ${canDelete && !(isLocked && !isCurrentUserAdmin) ? 'text-text-faint hover:text-red-400' : 'text-text-subtle opacity-40 cursor-not-allowed'}`}
          >
            <Trash2 size={13} />
          </button>
        </div>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-sm bg-card rounded-2xl shadow-xl shadow-black/40 border border-border p-6 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div>
                <Dialog.Title className="text-base font-semibold text-text">Delete Task</Dialog.Title>
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-text-subtle text-center py-6">No tasks yet.</p>
      ) : (
