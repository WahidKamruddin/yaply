    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm p-4">
      <div className="bg-card border border-border rounded-2xl shadow-2xl shadow-black/40 w-full max-w-xs">
        ...
        <div className="p-3 max-h-72 overflow-y-auto">
          {isLoading ? (
            <p className="text-xs text-text-subtle text-center py-4">Loading albums…</p>
          ) : albums.length === 0 && !showCreate ? (
            <p className="text-xs text-text-subtle text-center py-3">No albums yet.</p>
          ) : (
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl border transition-colors text-left ${
                    added === a.id
                      ...
                      : 'border-border hover:bg-tint text-text'
                  } disabled:opacity-60`}
                >
